...
class Relationships {
      ...
      Object.keys(this.models).forEach((key) => {
        ...
        if (this.models[key].length > 0 && key === 'hasMany') {
          if (this.hasCondition) {
            this.matching(query, key, (modelIndex) => {
              Relationships
                .hasMany(this.models[key][modelIndex][query], data, query, (results, err) => {
                  /* istanbul ignore if */
                  if (err) {
                    Relationships.resetBinding();
                    reject(err);
            ...
            this.matching(query, key, (modelIndex) => {
              Relationships
                .hasMany(this.models[key][modelIndex][query], data, null, (results, err) => {
                  /* istanbul ignore if */
                  if (err) reject(err);
                  else resolve(results);
                });
   ...
   * @memberof Relationships
   */
  static hasMany(model, data, storedQuery = null, callback) {
    if (storedQuery !== null) {
      HasMany
        .getWithCondition(data, model, storedQuery)
        .then((results) => {
  ...
  static resetBinding() {
    const Connection = require('../Connection/Connector');

    Connection.resetRelationshipBinding();
    this.insertGrammar = '';
    this.updateGrammar = '';
    this.deleteGrammar = '';
    this.whereGrammar = '';
