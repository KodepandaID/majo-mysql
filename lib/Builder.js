...
class Builder {
  ...
  static queryBuilder() {
    ...
    const query = `${select === 'SELECT *' ? `${select} ${from}` : select.concat(from).replace(/ {1,}/g, ' ')} ${Connector.joinGrammar === '' ? '' : `${Connector.joinGrammar} `}${Connector.whereGrammar === '' ? '' : `${Connector.whereGrammar} `}${Connector.orderByGrammar === '' ? '' : `${Connector.orderByGrammar} `}${Connector.groupByGrammar === '' ? '' : `${Connector.groupByGrammar} `}${Connector.havingGrammar === '' ? '' : `${Connector.havingGrammar} `}${mix === '' ? '' : `${mix} `}`;
    return query;
  }
