...
class Builder {
  ...
  static havingValueAndOperator(column, operator, value) {
    ...
    if (typeof value === 'object') {
      connector.bindings.having.push(`${column} ${operator} (${value.join(', ')})`);
    }
  }
  ...
  update(...args) {
    return new Promise((resolve, reject) => {
      ...
      connector.updateGrammar = `${connector.updateGrammar}${where !== '' ? ` ${where}` : ''}`;
  ...
  delete() {
    return new Promise((resolve, reject) => {
      ...
      connector.deleteGrammar = `${connector.deleteGrammar}${where !== '' ? ` ${where}` : ''}`;
