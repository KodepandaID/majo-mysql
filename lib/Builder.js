...
class Builder {
  ...
  orWhere(...args) {
    ...
    if (typeof args[0] === 'object' && args[0].length !== undefined) {
      args[0].forEach((tmp, index) => {
        if (index === 0) this.orWhere(...tmp);
        else this.andOrWhere(...tmp);
      });
    }
   ...
   * @memberof Builder
   */
  whereNull(column, whereClause = 'where') {
    if (whereClause === 'where') this[whereClause](column, null);
    else if (whereClause === 'orWhere' && Connector.bindings.orWhere.length === 0) this[whereClause](column, null);
    else if (whereClause === 'orWhere' && Connector.bindings.orWhere.length > 0) this.andOrWhere(column, null);
   ...
   * @memberof Builder
   */
  whereNotNull(column, whereClause = 'where') {
    if (whereClause === 'orWhere' && Connector.bindings.orWhere.length > 0) {
      const lastWhereIndex = Connector.bindings[whereClause].length - 1;
      const lastWhere = Connector.bindings[whereClause][lastWhereIndex];
      Connector.bindings[whereClause][lastWhereIndex] = `${lastWhere} AND ${column} IS NOT NULL`;
    } else Connector.bindings[whereClause].push(`${column} IS NOT NULL`);
   ...
   * @memberof Builder
   */
  whereEmptyString(column, whereClause = 'where') {
    Connector.bindings[whereClause].push(`${column} = ''`);
  ...
  static queryBuilder() {
    ...
    const query = `${select.concat(` ${from}`).replace(/ {1,}/g, ' ')} ${Connector.joinGrammar === '' ? '' : `${Connector.joinGrammar} `}${Connector.whereGrammar === '' ? '' : `${Connector.whereGrammar} `}${Connector.orderByGrammar === '' ? '' : `${Connector.orderByGrammar} `}${Connector.groupByGrammar === '' ? '' : `${Connector.groupByGrammar} `}${Connector.havingGrammar === '' ? '' : `${Connector.havingGrammar} `}${mix === '' ? '' : `${mix} `}`;
    return query;
  }
  ...
  hasRow(...args) {
    ...
    if (typeof args[0] === 'object') {
      args.forEach((tmp) => {
        Connector.relationshipBindingsQuery.push({ arguments: tmp, relationType: 'hasRow' });
      });
    } else {
      Connector.relationshipBindingsQuery.push({ arguments: args, relationType: 'hasRow' });
    }
      ...
      Connector.relationshipBindingsQuery.forEach((func) => {
        if (func.relationType !== 'hasRow') {
          ...
          relationshipsQuery.push({
            ...
            table: func.arguments[0],
            objectKey: func.arguments[1],
            objectReturnKey: func.arguments[2],
            foreignKey: func.arguments[3],
            localKey: func.arguments[4],
            relationType: func.relationType,
          });
        }
