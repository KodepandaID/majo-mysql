...
const RelationshipCondition = require('./Relationships/Condition');
...
class Builder {
  ...
  withCondition(callback) {
    Relationships.hasCondition = true;
    callback(RelationshipCondition);
        ...
        .then((results) => {
          if (!Relationships.hasRelationships) {
            ...
            Relationships
              .run(query, results)
              .then((finalResults) => {
                Relationships.removeStoredQuery(query);
