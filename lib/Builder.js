...
class Builder {
   ...
   * @memberof Builder
   */
  static whereArrayProcess(column, whereClauses = 'where', whereType = 'value') {
   ...
   * @memberof Builder
   */
  static whereValue(column, value, whereClauses = 'where', whereType = 'value') {
   ...
   * @memberof Builder
   */
  static whereValueAndOperator(column, operator, value, whereClauses = 'where', whereType = 'value') {
    const valueFinal = typeof value !== 'string' || whereType === 'column' ? value : `'${value}'`;
  ...
  hasOne(...args) {
    ...
    if (typeof args[0] === 'object') {
      Relationships.storedQuery.push(query);
      Relationships.hasRelationships = true;

      const models = { [query]: [] };
      args.forEach((tmp, index) => {
        const tmpModel = {
          table: tmp[0],
          objectName: tmp[1],
          foreignKey: tmp[2],
        ...
        models[query].push(tmpModel);

        if (args.length === (index + 1)) Relationships.models.hasOne.push(models);
      });
    } else {
      Relationships.storedQuery.push(query);
      Relationships.hasRelationships = true;
      ...
      Relationships.models.hasOne.push(model);
    }
  ...
  hasMany(...args) {
    ...
    if (typeof args[0] === 'object') {
      ...
      args.forEach((tmp, index) => {
        ...
        if (args.length === (index + 1)) Relationships.models.hasMany.push(models);
      });
    } else {
      Relationships.storedQuery.push(query);
      Relationships.hasRelationships = true;
      ...
      Relationships.models.hasMany.push(model);
    }
        ...
        .then((results) => {
          if (!Relationships.hasRelationships) {
            ...
            Relationships
              .run(query, results)
              .then((finalResults) => {
                Relationships.removeStoredQuery();
                Relationships.removeRelationshipsModel(query);
                Relationships.turnOffRelationships();
