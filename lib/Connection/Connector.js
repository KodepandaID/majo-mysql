...
class Connector {
  constructor() {
    ...
    this.relationshipBindingsQuery = [];
     ...
     * @var {boolean}
     */
    this.relationshipSelectDistinct = false;
  ...
  mysqlQueryProcess(query, callback) {
    const connection = mariadb.createConnection(this.connection);
  ...
  testConnection(callback) {
    ...
    connection.ping((error) => {
      if (error) {
        ...
        connection.end();
        callback({ response_time: `${new Date() - startTime} ms`, message: 'Connected' }, null);
      }
    });
  }
