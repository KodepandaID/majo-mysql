...
class Db {
  ...
  /**
   * Query to show information from spesific table
   *
   * @param {string} database
   * @param {string} table
   * @returns this
                      ...
                      .mysqlQueryProcess(rename.statement, (results, err) => {
                        ...
                        if (!err) {
                          this.dropDatabase(name)
                            ...
                            .catch(/* istanbul ignore next */ (errDrop) => {
                              reject(errDrop);
                            });
                        }
                      });
            ...
            .mysqlQueryProcess(queryCreateDatabaseArray.join(' '), (results, err) => {
              ...
              if (!err) {
                this.showDatabase(fromDatabase)
                  ...
                  .catch(/* istanbul ignore next */ (errShowDatabase) => {
    ...
    return new Promise((resolve, reject) => {
      Connector
        .mysqlQueryProcess(`SHOW CREATE DATABASE ${database}`, (results, err) => {
          /* istanbul ignore if  */
          if (err) reject(err);
          else resolve(results[0]['Create Database']);
        });
    });
  }
