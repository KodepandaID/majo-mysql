...
class Db {
  ...
  createUser(user, host, callback) {
    return new Promise((resolve, reject) => {
      ...
      connector.queries.push(`CREATE USER '${user}'@'${host}' ${connector.identified}; GRANT ${connector.grants.join(', ')} ON *.* TO '${user}'@'${host}' REQUIRE NONE WITH GRANT OPTION MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0; FLUSH PRIVILEGES;`);
