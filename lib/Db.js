...
class Db {
                ...
                this.showDatabase(fromDatabase)
                  .then((tables) => {
                    tables.forEach((data, index) => {
                      Db.showCreateTable(fromDatabase, data.TABLE_NAME)
                        .then((createTableQuery) => {
                          const queryCreateTableArray = createTableQuery.split(' ');
                          queryCreateTableArray[2] = `${toDatabase}.${data.TABLE_NAME}`;
                          connector
                            .mysqlQueryProcess(queryCreateTableArray.join(' '), (resultsCreateTable, errCreateTable) => {
  ...
  /**
   * Query to get query syntax to create like spesific database table
   *
   * @static
   * @param {string} database
   * @param {string} table
  ...
  static showCreateTable(database, table) {
    return new Promise((resolve, reject) => {
      connector
        .mysqlQueryProcess(`SHOW CREATE TABLE ${database}.${table}`, (results, err) => {
          if (err) reject(err);
          if (!err) resolve(results[0]['Create Table']);
        });
    });
  }
