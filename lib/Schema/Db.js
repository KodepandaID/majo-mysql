...
class Db {
  ...
  identified(password) {
    if (Connector.queries === 0) {
      Connector.identified = `IDENTIFIED VIA mysql_native_password USING '${password}'`;
    } else {
      const privileges = Connector.privileges.split('@');
      Connector.queries.push(`UPDATE mysql.user SET authentication_string = PASSWORD('${password}') WHERE User = ${privileges[0]} AND Host = ${privileges[1]};`);
    }
    return this;
  }
