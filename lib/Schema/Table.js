...
class Table {
  ...
  first() {
    ...
    connector.schemaBindings.commandModify.push(`${lastQuery} FIRST`);
  ...
  afterColumn(column) {
    ...
    connector.schemaBindings.commandModify.push(`${lastQuery} AFTER ${column}`);
    connector.schemaBindings.commandAdd.pop();
    return this;
  }
  ...
  change() {
    ...
    connector.schemaBindings.commandModify.push(lastQuery);
    connector.schemaBindings.commandAdd.pop();
    return this;
  }
  ...
  dropColumn(...args) {
    if (typeof args[0] === 'object') {
      args[0].forEach((column) => {
        connector.queries.push(`ALTER TABLE ${connector.schemaConnection === null ? `${connector.schemaTable}` : `${connector.schemaConnection}.${connector.schemaTable}`} DROP ${column}`);
      });
    }

    if (typeof args[0] === 'string') {
      connector.queries.push(`ALTER TABLE ${connector.schemaConnection === null ? `${connector.schemaTable}` : `${connector.schemaConnection}.${connector.schemaTable}`} DROP ${args[0]}`);
    }
  ...
  static pushToLastBinding(extraQuery) {
    ...
    connector.schemaBindings.commandAdd[lastBindingIndex] = `${lastQuery} ${extraQuery}`;
  }
}
