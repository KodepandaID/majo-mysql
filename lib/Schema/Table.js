...
class Table {
  ...
  afterColumn(column) {
    if (connector.schemaBindings.statement[0] === 'ALTER TABLE') {
      Table.pushToLastBinding(`AFTER ${column}`);
    }
    return this;
  }
  ...
  static addCommand(query) {
    if (connector.schemaBindings.statement[0] === 'CREATE TABLE') {
      connector.schemaBindings.command.push(`${query}`);
    }

    if (connector.schemaBindings.statement[0] === 'ALTER TABLE') {
      connector.schemaBindings.command.push(`ADD ${query}`);
    }
  }
  ...
  static pushToLastBinding(extraQuery) {
    const lastBindingIndex = (connector.schemaBindings.command.length - 1);
    const lastQuery = connector.schemaBindings.command[lastBindingIndex];

    connector.schemaBindings.command[lastBindingIndex] = `${lastQuery} ${extraQuery}`;
  }
}
