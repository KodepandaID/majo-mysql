...
class Table {
  ...
  unique(...args) {
    ...
    if (connector.schemaBindings.statement[0] === 'CREATE TABLE') {
      ...
      connector.schemaBindings.commandAdd.push(`UNIQUE (${column[0]})`);
    }

    if (connector.schemaBindings.statement[0] === 'ALTER TABLE') {
      connector.queries.push(`ALTER TABLE ${toSchema} ADD UNIQUE(${args.join(', ')})`);
    }
  ...
  primary(...args) {
    ...
    if (connector.schemaBindings.statement[0] === 'CREATE TABLE') {
      const last = Table.getQueryLastBinding();
      const column = last.query.split(' ');
      connector.schemaBindings.commandAdd.push(`PRIMARY KEY (${column[0]})`);
    }

    if (connector.schemaBindings.statement[0] === 'ALTER TABLE') {
      if (lastQuery.includes('DROP PRIMARY KEY')) {
        Table.pushToLastBinding(`, ADD PRIMARY KEY(${args[0]})`);
      } else {
        connector.queries.push(`ALTER TABLE ${toSchema} ADD PRIMARY KEY(${args.join(', ')})`);
      }
    }
  ...
  spatial(...args) {
    ...
    if (connector.schemaBindings.statement[0] === 'CREATE TABLE') {
      ...
      connector.schemaBindings.commandAdd.push(`SPATIAL (${column[0]})`);
    }

    if (connector.schemaBindings.statement[0] === 'ALTER TABLE') {
      connector.queries.push(`ALTER TABLE ${toSchema} ADD SPATIAL(${args.join(', ')})`);
    }
  ...
  dropIndex(column) {
    const toSchema = Table.getSchema();
    connector.queries.push(`ALTER TABLE ${toSchema} DROP INDEX ${column}`);
    return this;
  }
  ...
  index(...args) {
    if (connector.schemaBindings.statement[0] === 'CREATE TABLE') {
      const last = Table.getQueryLastBinding();
      const column = last.query.split(' ');
      connector.schemaBindings.commandAdd.push(`INDEX (${column[0]})`);
    }

    if (connector.schemaBindings.statement[0] === 'ALTER TABLE') {
      const toSchema = Table.getSchema();
      connector.queries.push(`ALTER TABLE ${toSchema} ADD INDEX(${args.join(', ')})`);
    }
   ...
   * @memberof Table
   */
  static getQueryLastBinding() {
    const lastBindingIndex = (connector.schemaBindings.commandAdd.length - 1);
    const lastQuery = connector.schemaBindings.commandAdd[lastBindingIndex];
   ...
   * @memberof Table
   */
  static pushToLastBinding(extraQuery) {
    const last = Table.getQueryLastBinding();

    connector.schemaBindings.commandAdd[last.lastIndex] = `${last.query} ${extraQuery}`;
  }
}
