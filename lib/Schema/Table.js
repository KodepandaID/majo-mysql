...
class Table {
  ...
  afterColumn(column) {
    if (connector.schemaBindings.statement[0] === 'ALTER TABLE') {
      const lastBindingIndex = (connector.schemaBindings.parenthesis.length - 1);
      const lastQuery = connector.schemaBindings.parenthesis[lastBindingIndex];

      connector.schemaBindings.parenthesis[lastBindingIndex] = `${lastQuery} AFTER ${column}`;
    }
    return this;
  }
  ...
  autoIncrement() {
    ...
    connector.schemaBindings.parenthesis[lastBindingIndex] = `${lastQuery} AUTO_INCREMENT PRIMARY KEY`;
    return this;
  }
